...
function BranchRow({
  ...
  return (
    <article className="grid grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] items-center gap-4 border-t border-neutral-200 py-8 sm:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] sm:gap-8 sm:py-10 lg:gap-12 lg:py-12">
      <div className="min-w-0 border-l border-neutral-300 pl-3 sm:pl-5">
        <p className="text-[9px] font-medium tracking-[0.16em] text-neutral-500 uppercase sm:text-[10px] sm:tracking-[0.2em]">
          Branch {index + 1}
        </p>
        <p className="mt-2 text-[11px] font-semibold leading-snug text-neutral-900 sm:text-sm">
          {outlet.locationLine}
        </p>
        <p className="mt-2 text-[10px] leading-relaxed tracking-[0.04em] text-neutral-600 sm:text-[11px] sm:tracking-[0.06em]">
          Branch Incharge:{" "}
          <a
            href={`tel:${outlet.phone}`}
            className="font-medium text-neutral-900 transition-colors hover:text-neutral-600 focus-visible:ring-2 focus-visible:ring-neutral-900 focus-visible:ring-offset-1"
          >
            {outlet.phoneDisplay}
          </a>
        </p>
      ...
      <div className="mx-auto max-w-[1440px]">
        <ScrollReveal variant="slideInLeft">
          <header className="max-w-md">
            ...
            <h2
              id="outlets-heading"
              className="mt-6 text-[clamp(1.75rem,3.2vw,2.65rem)] font-normal leading-[1.12] tracking-[-0.01em] text-[#0F0F0F]"
            >
              A Space That <span className="font-bold">Inspires</span> Your{" "}
              <span className="font-bold">Luxury Home</span>
            </h2>
            <p className="mt-8 text-[13px] leading-relaxed text-neutral-600 sm:text-sm">
