...
const actionButtonClass =
  "inline-flex h-16 w-full items-center justify-center gap-2.5 rounded-none border border-gold bg-transparent px-8 text-[12px] font-medium uppercase tracking-[0.08em] text-zinc-900 transition-colors duration-300 hover:border-zinc-900 hover:bg-zinc-900 hover:text-white sm:w-fit";
  ...
  return (
    <div className="mt-8 flex w-full flex-col items-start gap-4 sm:mt-10">
      ...
      <div className="flex w-full flex-col items-start gap-2 sm:w-auto">
        ...
        <p className="pl-1 text-[12px] tracking-wide text-zinc-800">
          {outlet.phoneDisplay}
        </p>
      </div>
      <a
  ...
  return (
    <figure className="relative m-0 min-h-[62vw] w-full overflow-hidden md:min-h-0 md:h-full">
      <Image
        ...
        className={`block object-cover ${imageLiftClass}`}
      />
      <ImageWash />
    </figure>
  );
...
function OutletInfo({ outlet }: { outlet: Outlet }) {
  return (
    <div className="flex w-full flex-col justify-center border-y border-zinc-200 bg-white px-10 py-16 md:h-full md:border-y-0 md:px-[9vw] md:py-[8vw]">
      <p className="text-[11px] font-medium uppercase tracking-[0.22em] text-gold">
        MRF Galaxy Outlet
      </p>

      <h2 className="mt-5 font-semibold tracking-tight text-zinc-950 [font-size:clamp(32px,3vw,56px)]">
        {outlet.name}
      </h2>

      <address className="mt-6 max-w-[20rem] not-italic">
        <p className="flex items-start gap-2 text-[12px] leading-relaxed text-zinc-800 sm:text-[13px]">
          <MapPinIcon />
          <span>
